...
export default function BrowseScreen() {
  ...
  const onRefresh = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to refresh mangas", err);
    } finally {
      setRefreshing(false);
    }
  }, [searchQuery, genreFilters, sortField, sortDirection]);
  ...
  const activeGenrePreview = useMemo(() => {
    const activeEntries = Object.entries(genreFilters).filter(
      ([_, s]) => s !== "none",
    );
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={styles.dropdownsRow}>
        ...
        >
          <Text
            style={[styles.dropdownLabel, { color: theme.text }]}
            numberOfLines={1}
          >
            {activeGenrePreview}
          </Text>
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => setIsGenreOpen(false)}
      >
        <TouchableWithoutFeedback onPress={() => setIsGenreOpen(false)}>
          <View style={styles.modalOverlay}>
            <TouchableWithoutFeedback>
              <View
                style={[styles.modalContent, { backgroundColor: "#1A1D23" }]}
              >
      ...
      {loading && mangas.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <MangaCard
              manga={item}
              onPress={(id) => router.push(`/manga/${id}` as any)}
              numColumns={2}
            />
          )}
